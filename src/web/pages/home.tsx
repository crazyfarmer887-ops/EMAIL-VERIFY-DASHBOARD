...
export default function HomePage() {
  ...
  return (
    <div style={{ padding: '20px 16px 0' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button
            ...
            style={{
              ...
              padding: '8px 14px', fontSize: 13, color: '#7C3AED',
      ...
      {/* Category Grid */}
      <h2 style={{ fontSize: 15, fontWeight: 700, color: '#1E1B4B', margin: '0 0 12px' }}>
        서비스별 최저가
      </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                key={cat.key}
                onClick={() => navigate(`/price/${cat.key}`)}
                style={{
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  <img src={cat.logo} alt={cat.label} style={{ width: 28, height: 28, objectFit: "contain", borderRadius: 6 }} onError={(e)=>{(e.target as HTMLImageElement).style.display="none"}} />
                  <span style={{
                    fontSize: 11, fontWeight: 700, color: cat.color,
                    background: cat.bg, borderRadius: 6, padding: '2px 6px',
                  }}>
                    {cat.label.length > 5 ? cat.label.slice(0,5) + '..' : cat.label}
                  </span>
                </div>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#1E1B4B', lineHeight: 1 }}>
                  {summary?.lowestPricePerDayNum
                    ? `${summary.lowestPricePerDayNum.toLocaleString()}원`
                    : '-'}
                </div>
      ...
      <div style={{ marginTop: 24, marginBottom: 8 }}>
        ...
        >
          ...
          <div style={{ marginLeft: 'auto', color: '#A78BFA', fontSize: 18 }}>›</div>
        </button>
      </div>
